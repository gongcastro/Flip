...
data %>%
  ...
  ggsave(here("Figures", "01_lookingtimes-study.png"), height = 5, width = 12)
...
ggplot(data = filter(anova, Term != "(Intercept)"), aes(Term, Coefficient)) +
  ...
  ggsave(here("Figures", "03_coefficients.png"), height = 4)
...
ggplot(effects, aes(x = HPP, y = fit, linetype = Item)) +
  ...
  scale_y_continuous(limits = c(5000, 7750), breaks = seq(5000, 7750, by = 500)) +
...
write(post.pred.p, here("Data", "02_results-posterior-5.txt"))
