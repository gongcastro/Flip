...
# import data
data      <- read.table(here("Data/HPP3", "01_data-processed-3.csv"), header = TRUE, sep = ",", dec = ".")
anova     <- read.table(here("Data/HPP3", "02_results-lmem-3.csv"), header = TRUE, sep = ",", dec = ".")
effects   <- read.table(here("Data/HPP3", "02_results-effects-3.csv"), header = TRUE, sep = ",", dec = ".")
posterior <- scan(here("Data/HPP3", "02_results-posterior-3.txt"))
tolerance <- read.table(here("Data/HPP3", "02_results-multicollinearity-3.csv"), header = TRUE, sep = ",", dec = ".")
...
We retrieved data from `r n_participants$n` participants: `r n_study$n[1]` from @santolin2019a, `r n_studyLocation$n[2]` tested in Wisconsin, `r n_studyLocation$n[1]` tested in the replication study in Barcelona [@santolin2019], and `r n_studyLocation$n[3]` from @saffran2003, tested in Wisconsin. Two datapoints were available for each participant: one for the mean looking time in "familiar" trials, and one for the mean looking time in "novel" trials. A total of `r n$n` datapoints were included in the analysis. Participants included in our analysis were those included in the final version of both studies.
...
include_graphics(here("Figures/HPP3", "01_lookingtimes-study.png"))
...
include_graphics(here("Figures/HPP3", "02_lookingtimes-hpp.png"))
...
include_graphics(here("Figures/HPP3", "03_interaction.png"))
```
...
$$ \begin{aligned}LT_{ips} = \\ &\beta_{0} + Participant_{0p} + Study_{0s} + \\ &\beta_{1} \times HPP_{i} + \\ &\beta_{2} \times Item_{i} + \\ &\beta_{3} \times (HPP \times Item{i}) + \\ &e_{ips},e_{ips} \sim N(0, \sigma^2), Participant_{0p} \sim N(0, \tau_{00}^2), Study_{0p} \sim N(0, &\omega_{00}^2)\end{aligned}$$
...
include_graphics(here("Figures/HPP3", "04_model-assumptions-normality.png"))
...
include_graphics(here("Figures/HPP3", "04_model-assumptions-homoskedasticity.png"))
